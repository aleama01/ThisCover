...
const AlbumCard = ({ album }: { album: IAlbum }) => {
  ...
  return (
    <div className='album-card m-auto lh-1 '>
      ...
      <div className='d-flex w-100 align-items-center fs-12 fw-200'>
        <img src={`${album.user.image}`} width={30} height={30} className="circle" />
        <div className='ms-2'>{album.user.name}</div>
        <div className='justify-self-end ms-auto text-gray'>{timeLeft(album.time_left)}</div>
      </div>
      ...
      <div className='w-100 d-flex flex-column fs-18 mt-3'>
        <button type='button' className='btn-accent fw-300 py-2 position-relative'>
